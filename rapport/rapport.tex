...
\footnotetext[3]{SSII : Société de service en ingénierie informatique}
...
\footnotetext[3]{W3C (World Wide Web Consortium) est un organisme de normalisation à but non lucratif chargé de promouvoir la compatibilité des technologies}
...
CubicWeb respecte les standards ouverts d'Internet afin de simplifier les communications et les échanges. Elle suit les standards du World Wide Web Consortium\footnotemark[3] (RDF\footnotemark[1], SPARQL\footnotemark[2], HTML5\footnotemark[3], CSS3\footnotemark[4], Responsive Design\footnotemark[5]) et peut gérer nativement plusieurs modèles de données faisant office de standards de faits (FOAF\footnotemark[6], SIOC\footnotemark[7], DOAP\footnotemark[8]).

\footnotetext[1]{RDF (Resource Description Framework) est un modèle de graphe destiné à décrire de façon formelle les ressources Web et leurs métadonnées, de façon à permettre le traitement automatique de ces descriptions.}
\footnotetext[2]{SPARQL est un langage de requêtes et un protocole qui permet de rechercher, d'ajouter, de modifier ou de supprimer des données RDF disponibles à travers Internet.}

\footnotetext[3]{HTML5 est la cinquième version du format de données HTML conçu pour représenter les pages Web}
\footnotetext[4]{CSS3 est un langage servant à décrire la présentation des documents HTML et XML.}

\footnotetext[5]{Responsive Design est une notion de conception de sites Web qui permet à l'utilisateur de consulter le même site Web à travers une large gamme d'appareils (moniteurs d'ordinateur, smartphones, tablettes, TV, etc.) avec le même confort visuel.}

\footnotetext[6]{FOAF est un vocabulaire RDF permettant de décrire des personnes et les relations qu’elles entretiennent entre elles. C'est une application du Web sémantique.}

\footnotetext[7]{SIOC est un vocabulaire utilisant RDF permettant de décrire des objets couramment utilisés sur les sites communautaires et leurs relations. SIOC est une application du Web sémantique pour décrire des blogs, des forums, des wikis... }

\footnotetext[8]{DOAP est un vocabulaire RDF décrivant les projets logiciels}
...
\captionof{figure}{Modification du code existant de génération de tables SQL afin de ne pas générer les tables correspondantes aux relations réécrites}
\end{center}
